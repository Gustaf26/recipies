...
class AllRecipiesTwo {
	...
	addToDbTwo = (recipie) =>db.collection('papa').add({
	...
	.catch(err => {
	 ...
	 getRecipiesTwo = () => {
		...
		db.collection('papa').get().then(snapshot => {
...
autoresEl.addEventListener('click', e => {
    ...
    if (e.target.id == "lela") {recipiesEl.toggleAttribute('hidden');
    }
	else if (e.target.id == "papa") {recipiesTwoEl.toggleAttribute('hidden')}
	else if (e.target.id == "suecas") {recipiesThreeEl.toggleAttribute('hidden')}
})
